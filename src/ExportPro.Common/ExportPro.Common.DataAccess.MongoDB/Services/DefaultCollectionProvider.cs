...
public class DefaultCollectionProvider(IMongoDbConnectionFactory connectionFactory) : ICollectionProvider
{
    ...
    public IMongoCollection<TDocument> GetCollection<TDocument>(string? name = null) where TDocument : IModel
    {
    ...
    protected virtual string GetCollectionName(Type type) => $"{type.Name}";
}
