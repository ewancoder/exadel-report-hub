...
public static class PermissionEvaluator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (userRole == Role.SuperAdmin &&
            (request.Resource == Resource.Clients || request.Resource == Resource.Users))
            return;
        ...
        if (request.ClientIds != null && request.ClientIds.Count > 0)
        {
            ...
            {
                var allowed = await permissionCheck(userId.ToGuid(), clientId, request.Resource, request.Action, cancellationToken);
                if (!allowed)
                {
        ...
        {
            var allowed = await permissionCheck(userId.ToGuid(), null, request.Resource, request.Action, cancellationToken);
            if (!allowed)
            {
                throw new UnauthorizedAccessException(
