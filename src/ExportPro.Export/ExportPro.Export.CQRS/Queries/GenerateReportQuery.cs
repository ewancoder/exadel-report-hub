...
) : IRequestHandler<GenerateReportQuery, ReportFileDto>
{
    public async Task<ReportFileDto> Handle(GenerateReportQuery request, CancellationToken cancellationToken)
    {
        ...
        List<InvoiceDto> invoices =
            request.Filters.ClientIds?.Any() == true
                ? allInvoices
                    .Where(i => i.ClientId.HasValue && request.Filters.ClientIds!.Contains(i.ClientId.Value))
                    .ToList()
        ...
        {
            var dayStart = d.Date;
            invoices = invoices.Where(i => i.IssueDate.Date >= dayStart).ToList();
        }

        var (itemsByClient, plansByClient) = await FetchItemsAndPlansAsync(request.Filters, cancellationToken);
    ...
    private async Task<(
        Dictionary<Guid, List<ItemResponse>>,
        Dictionary<Guid, List<PlansResponse>>
    )> FetchItemsAndPlansAsync(ReportFilterDto f, CancellationToken ct)
    {
        var ids =
            f.ClientIds?.Any() == true ? f.ClientIds!
            : f.ClientId is { } single && single != Guid.Empty ? [single]
            : [];
        ...
        var tasks = ids.Select(async id =>
        {
            ...
            return (id, items.Data ?? [], plans.Data ?? []);
        });
    ...
    )
    {
        ...
        return (itemsTask.Result.Data ?? [], plansTask.Result.Data ?? []);
    }
