...
) : IRequestHandler<GenerateReportQuery, ReportFileDto>
{
    ...
    )
    {
        ...
        var reportContentDto = await RetrieveClientNameAsync(
            request,
            clientId,
            invoices,
            items,
    ...
    {
        var invoices = clientId != Guid.Empty ? allInvoices.Where(i => i.ClientId == clientId).ToList() : allInvoices;
        return invoices;
    }

    private async Task<List<InvoiceDto>> FetchInvoicesAsync(CancellationToken cancellationToken)
    {
        return (await storageApi.GetInvoicesAsync(1, int.MaxValue, false, cancellationToken)).Data?.Items ?? [];
    }
