    ...
    : ICommandHandler<ImportCustomersCommand, int>
{
        ...
        ParseExcelAsync(IFormFile excelFile)
    {
        ...
        logger.LogInformation("Starting to parse Excel file: {FileName}, size: {FileSize} bytes",
        ...
        try
        {
            ...
            logger.LogInformation("Successfully parsed Excel file with {CustomerCount} customers",
                customers.Count);
        ...
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("Starting to save {CustomerCount} customers to database",
            customers.Count);

        try
        {
            var result = await storageApi.CreateCustomersBulkAsync(customers, cancellationToken);
            logger.LogInformation("Successfully imported {ImportedCount} customers",
                result.Data);
            return result;
        }
        ...
        {
            logger.LogWarning(ex, "Bad request error while importing customers: {ErrorContent}",
                ex.Content);
