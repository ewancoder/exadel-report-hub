...
public class AuthTests
{
    ...
    private void SettingHttpContext()
    {
        ...
        mockRequestCookies
            .TryGetValue("refreshToken", out Arg.Any<string?>())
            .Returns(x =>
            {
                x[1] = "refreshToken";
                return true;
        ...
        _authController.ControllerContext = new ControllerContext { HttpContext = mockHttpContext };
    }
    ...
    private void SettingDtos()
    {
        ...
        _loginDto = new UserLoginDto { Email = "TestUser2@example.com", Password = "TestUser2@example.com" };
    }
    ...
    public async Task Register_WhenUserIsValid_ReturnsOkAndSetsRefreshTokenCookie()
    {
        ...
        _mediator
            .Send(Arg.Is<RegisterCommand>(obj => obj.RegisterDto.Equals(_registerDto)), Arg.Any<CancellationToken>())
            .Returns(response);
        //Act
        ...
        _mockResponseCookies
            ...
            .Append(
                "refreshToken",
                _authResponseDto.RefreshToken,
                Arg.Is<CookieOptions>(options =>
                    options.HttpOnly && options.Secure && options.Expires == _authResponseDto.ExpiresAt
                )
            );
    ...
    public async Task Login_WhenUserIsValid_ReturnsOkAndSetsRefreshTokenCookie()
    {
        ...
        _mediator
            .Send(Arg.Is<LoginCommand>(obj => obj.LoginDto.Equals(_loginDto)), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(response));
        ...
        _mockResponseCookies
            ...
            .Append(
                ...
                _authResponseDto.RefreshToken,
                Arg.Is<CookieOptions>(options =>
                    options.HttpOnly == true && options.Secure == true && options.Expires == _authResponseDto.ExpiresAt
                )
            );
    }
    ...
    public async Task Login_WhenUserIsNotValid_ReturnsUnauthorized()
    {
        ...
        _mediator
            .Send(
                Arg.Is<LoginCommand>(obj =>
                    obj.LoginDto.Email == _loginDto.Email && obj.LoginDto.Password == _loginDto.Password
                ),
    ...
    public async Task Logout_WhenRefreshTokenExists_ReturnsOkAndDeletesRefreshTokens()
    {
        ...
        await _mediator
            .Received(1)
            .Send(Arg.Is<LogoutCommand>(obj => obj.RefreshToken is string), Arg.Any<CancellationToken>());
