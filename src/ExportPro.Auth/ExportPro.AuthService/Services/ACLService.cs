...
public sealed class ACLService(IACLRepository aclRepository) : IACLService
{
    ...
    public Task<bool> UpdateUserRole(
        ObjectId userId,
        ObjectId clientId,
        UserRole newRole,
        CancellationToken cancellationToken = default
    ...
    {
        return aclRepository.UpdateUserClientRoleAsync(userId, clientId, newRole, cancellationToken);
    }

    public Task<List<UserClientRoles>> GetAccessibleUserRolesAsync(
        ObjectId userId,
        CancellationToken cancellationToken = default
    )
    {
        return aclRepository.GetClientIdsForUserAsync(userId, cancellationToken);
    }

    public Task DeleteAllRoles(ObjectId userId, CancellationToken cancellationToken = default)
    {
        return aclRepository.DeleteRolesAsync(userId, cancellationToken);
    }

    private async Task<List<UserClientRoles>> Roles(
        ObjectId userId,
        ObjectId clientId,
