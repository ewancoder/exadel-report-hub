...
public record CreateUserCommand(string Username, string Email, string Password, Guid? ClientId, UserRole? UserRole)
    : ICommand<Guid>,
    ...
    public async Task<BaseResponse<Guid>> Handle(CreateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.ClientId != null && request.UserRole != null)
            await aclService.GrantPermission(
                createdUser.Id,
                ((Guid)request.ClientId).ToObjectId(),
                (UserRole)request.UserRole,
                cancellationToken
            );

        return new SuccessResponse<Guid>(user.Id.ToGuid(), "User Created Successfully");
    }
}
