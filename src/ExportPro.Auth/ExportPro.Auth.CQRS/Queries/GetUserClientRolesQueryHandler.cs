...
public sealed class GetUserClientRolesQueryHandler(IACLService aCLService, IHttpContextAccessor httpContextAccessor)
    : IQueryHandler<GetUserClientRolesQuery, List<UserClientRolesDTO>>
{
    public async Task<BaseResponse<List<UserClientRolesDTO>>> Handle(
        GetUserClientRolesQuery request,
    ...
    )
    {
        ...
        var clientRolesDto = clientRoles
            .Select(cr => new UserClientRolesDTO { ClientId = cr.ClientId.ToGuid(), Role = cr.Role })
            .ToList();
