...
public class ReportScheduleDtoValidator : AbstractValidator<ReportScheduleDto>
{
    public ReportScheduleDtoValidator()
    {
        RuleFor(x => x.Time)
            ...
            .WithMessage("Time must be a valid time in 24-hour format (00:00 to 23:59).");

        RuleFor(x => x.Frequency).IsInEnum().WithMessage("Frequency is not valid.");
    ...
    private bool BeValidTimeOnly(TimeOnly time) =>
        time.Hour >= 0 && time.Hour < 24 && time.Minute >= 0 && time.Minute < 60;
}
