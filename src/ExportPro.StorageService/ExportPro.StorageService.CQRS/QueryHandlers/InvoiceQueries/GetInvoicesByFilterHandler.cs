...
public sealed class GetInvoicesByFilterHandler(IInvoiceRepository repository, ICustomerRepository customerRepository, ICountryRepository countryRepository, ICurrencyRepository currencyRepository, IMapper mapper)
    : IQueryHandler<GetInvoicesByFilter, PaginatedList<InvoiceDto>>
{
    ...
    )
    {
        ...
        {
            var customerId = await customerRepository.GetOneAsync(x => x.Name == customer.Name && !x.IsDeleted, cancellationToken);
            if (customerId != null)
            {
        ...
        {
            var customerId = await customerRepository.GetOneAsync(x => x.Address == customer.Address && !x.IsDeleted, cancellationToken);
            if (customerId != null)
            {
                invoicesDto = [.. invoicesDto.Where(x => x.CustomerId.ToObjectId() == customerId.Id)];
            }
        ...
        {
            var customerId = await customerRepository.GetOneAsync(x => x.Email == customer.Email && !x.IsDeleted, cancellationToken);
            if (customerId != null)
            {
                invoicesDto = [.. invoicesDto.Where(x => x.CustomerId.ToObjectId() == customerId.Id)];
            }
        ...
        {
            var countryId = await countryRepository.GetOneAsync(x => x.Id.ToGuid() == customer.CountryId && !x.IsDeleted, cancellationToken);
            if (countryId != null)
            {
                var customerId = await customerRepository.GetOneAsync(x => x.CountryId == countryId.Id && !x.IsDeleted, cancellationToken);
                if (customerId != null)
                {
                    invoicesDto = [.. invoicesDto.Where(x => x.CustomerId.ToObjectId() == customerId.Id)];
                }
