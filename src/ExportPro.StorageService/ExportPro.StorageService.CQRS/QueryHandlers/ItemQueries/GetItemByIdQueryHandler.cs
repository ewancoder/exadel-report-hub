...
public class GetItemByIdQueryHandler(IClientRepository repository, IMapper mapper)
    : IQueryHandler<GetItemByIdQuery, ItemResponse>
{
    public async Task<BaseResponse<ItemResponse>> Handle(GetItemByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var item = client.Items?.FirstOrDefault(i => i.Id == request.ItemId.ToObjectId() && !i.IsDeleted);
